...
\subsubsection{Analysis in the $(\ur, \ell)$ coordinates}
...
\begin{equation}
  x = A \frac{(z-\tfrac{1}{b^2})(z-b)}{(z-1)(z-\tfrac{1}{b})}, \quad y = B \frac{(z-1)}{(z-b)(z-\tfrac{1}{b})},
\end{equation}
with
\begin{equation}
...
\subsubsection{Differential equations satisfied by Kerr--Doran $Q$}

Starting from the Picard--Fuchs system in the $(\ell,\ur)$ coordinates given in \cref{eq:PicardFuchsD1D2}, we change variables to the Kerr--Doran parameter $b$ using \cref{eq:elur-in-terms-of-b}. We also introduce a small displacement variable $\delta\ur$ transverse to the discriminant locus, so that
\[
  \ell=\ell(b),\qquad \ur=\ur(b)+\delta\ur.
\]
Rewriting the Picard--Fuchs operators in the coordinates $(b,\delta\ur)$ and expanding near the discriminant to leading order in $\delta\ur$, we expand a local period in the form
\[
  G(b,\delta\ur)=G_0(b)+G_1(b)\,\delta\ur+\mathcal{O}(\delta\ur^2).
\]
At leading order, the transformed Picard--Fuchs equations involve $G_1(b)$ and $G_1'(b)$. However, by taking an appropriate linear combination of the two equations, these auxiliary terms can be eliminated, yielding a single second-order ODE for the cusp period $G_0(b)$:
\begin{equation}\label{eq:KD-G0-ode}
  G_0''(b)+\frac{b^4-4 b^3-8 b^2-6 b-1}{b\,(b^4+b^3-b-1)}\,G_0'(b)=0.
\end{equation}


\subsubsection{Domain, range, and branching structure of the map $\ell(b)$}
...
\end{equation}
so $\ell$ factors through the $\ZZ_2$-quotient $\PP^1_b/\langle b\mapsto 1/b\rangle\cong\PP^1_u$ via the standard invariant coordinate
\begin{equation}
  u \;:=\; b+\frac{1}{b}.
\end{equation}
Using the identities $b^2+b+1=b(u+1)$ and $(b+1)^8=b^4(u+2)^4$, one obtains the compact expression
\begin{equation}\label{eq:l-of-u}
  \ell(b)= -\frac{(u+1)^3}{(u+2)^4},
  \qquad u=b+\frac{1}{b}.
\end{equation}
Hence the map decomposes as
\begin{equation}
  \PP^1_b \xrightarrow{\ \deg 2\ } \PP^1_u \xrightarrow{\ \deg 4\ } \PP^1_\ell,
\end{equation}
...
% \tree{\cO(-1,-1)\sh{1},\,\{\cO(1,1),\,\{2\,\cO(0,0)\sh{1},\,2\,\cO(1,0)\}\}}
\ctree{2}\mtreex{%
...
\subsection{Degenerations of the map $\ur \mapsto J$}
...
\subsubsection{Distinguished special values of $\ell$ (explicit list)}
...
\subsubsection{Behavior at $u=\infty$ and additional special values}
...
\subsubsection{Periods near $\ur = -\frac{8}{9}$}
